...
`pruneProcessedHistoryImages` now takes an optional `HistoryImagePrunePolicy` (`cacheRetention`, `lastCacheTouchAt`, `maxDeferredImageTurns` default 8, `sessionManager`). Pruning is a view transform (stored history keeps its images), so the boundary is remembered in the session as an `openclaw.image-prune-boundary` custom entry. While the provider prompt cache is warm (retention `long` = 1h, `short` = 5m, measured from the session's last `openclaw.cache-ttl` entry) the boundary stays frozen where the last cold request put it; it advances once the cache is cold, or once `maxDeferredImageTurns` prunable turns have accumulated behind it, or after compaction shrinks the history. `attempt.ts` resolves the policy from `effectivePromptCacheRetention` and `readLastCacheTtlTimestamp` at both the pre-prompt prune and the in-loop `transformContext` hook. Behaviour without a policy is unchanged. Candidate for upstream.

## extensions/signal/src/client.ts — no header deadline on the SSE event stream

signal-cli 0.14.1 does not flush the `/api/v1/events` response headers until the first event, so upstream's 10s header deadline (new in v2026.4.26) tears the stream down and reconnects forever; Signal inbound never works. `streamSignalEvents` now defaults to no header deadline (`timeoutMs` 0 disables it), which is the pre-4.26 behaviour; explicit `timeoutMs` values still apply.
